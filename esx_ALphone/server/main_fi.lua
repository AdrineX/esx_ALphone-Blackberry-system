...
AddEventHandler('esx_phone:startCall', function(number)
  ...
  if foundPlayer ~= false then
    ...
    TriggerClientEvent('esx_phone:endCall', _source, 'Kontakti ei ole paikalla.')
  end
    ...
    function(result)
      ...
      if foundNumber then
        ...
        if phoneNumber == xPlayer.get('phoneNumber') then
          TriggerClientEvent('esx:showNotification', _source, 'Et voi lisätä itseäsi')
        else
          ...
          if hasAlreadyAdded then
            TriggerClientEvent('esx:showNotification', _source, 'Tämä kontakti on jo puhelimessa')
          else
            ...
            MySQL.Async.execute(
              ...
              function(rowsChanged)
                ...
                TriggerClientEvent('esx:showNotification', _source, 'Kontakti lisättiin')
        ...
        TriggerClientEvent('esx:showNotification', source, 'Tämä numero ei ole olemassa')
      end
...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  TriggerClientEvent('esx:showNotification', _source, 'Puhelu maksoi sinulle ~r~$' .. amount .. '~s~')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'Sinä siirsit ~g~$' .. amount .. '~s~ pelaajalle ' .. targetXPlayer.name)

  else
    TriggerClientEvent('esx:showNotification', _source, 'Virheellinen summa')
  end
